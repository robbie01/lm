...
;;; than one of itself. One pair of eyes is one record, and every function
;;; here is handed it.
...
;; dragged and the eyes go with it.
(define (place-eyes e)
...
;; bitmap, whatever task is calling.
...
(define (draw-all-1 e rp)
  ...
  ;; true any more; track draws only what moved.
  (set-eyes-px1! e -1)
  (set-eyes-py1! e -1)
  (set-eyes-px2! e -1)
  (set-eyes-py2! e -1)
...
;; white it sat on, the new one drawn, and the square round both handed to
;; the compositor. `pupil-at` keeps a pupil two pixels inside the white, so
;; painting one out never reaches the outline.
...
;; ---------------------------------------------------------------- the app
;; Once a frame: look, move whichever pupil has moved, and wait for the next
;; frame.
(define (eyes-task e)
  (while t
    (track e)
...
;; refresh callback and the task's body both close over the record, which is
;; how they know which pair they are.
(define (eyes . opts)
  (let* ((e (eyes-alloc))
         (n (length opts))
