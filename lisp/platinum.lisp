...
(define (pt-hline x y w c) (fill-rect x y w 1 c))
(define (pt-vline x y h c) (fill-rect x y 1 h c))

(define (pt-frame x y w h c)
  (pt-hline x y w c)
  (pt-hline x (%+ y (%- h 1)) w c)
  (pt-vline x y h c)
  (pt-vline (%+ x (%- w 1)) y h c))

(define (pt-raised x y w h)
  (pt-hline x y (%- w 1) pt-white)
  (pt-vline x y (%- h 1) pt-white)
  (pt-hline (%+ x 1) (%+ y (%- h 1)) (%- w 1) pt-g6)
  (pt-vline (%+ x (%- w 1)) (%+ y 1) (%- h 1) pt-g6))

(define (pt-sunken x y w h)
  (pt-hline x y (%- w 1) pt-g6)
  (pt-vline x y (%- h 1) pt-g6)
  (pt-hline (%+ x 1) (%+ y (%- h 1)) (%- w 1) pt-white)
  (pt-vline (%+ x (%- w 1)) (%+ y 1) (%- h 1) pt-white))
        ...
        (let ((cx x) (end (%+ x w)))
          (dolist (c cuts)
            (let ((c0 (if dark (%car c) (%- (%car c) 1)))
              ...
              (if (%> c0 cx)
                  (pt-hline cx py (%- (if (%< c0 end) c0 end) cx)
                            (if dark pt-g8 pt-white))
                  nil)
              (if (%> c1 cx) (set! cx c1) nil)))
          (if (%< cx end)
              (pt-hline cx py (%- end cx) (if dark pt-g8 pt-white))
              nil)))
      (set! row (%+ row 1)))
    nil))
...
(define (pt-title-box x y kind)
  ;; kind 0 close, 1 zoom, 2 collapse.
  (fill-rect x y 12 12 pt-g7)
  (fill-rect (%+ x 1) (%+ y 1) 10 10 pt-g13)
  (let ((row 0))
    (while (%< row 8)
      (let ((col 0))
        (while (%< col 8)
          (let ((d (%+ row col)))
            (plot (%+ x (%+ col 2)) (%+ y (%+ row 2))
                  (pt-grey (%+ 9 (%/ (%* d 6) 14)))))
          (set! col (%+ col 1))))
      (set! row (%+ row 1))))
  ...
  ;; nothing for close.
  (if (%= kind 1)
      (pt-vline (%+ x 6) (%+ y 2) 8 pt-g13)
      (if (%= kind 2) (pt-hline (%+ x 2) (%+ y 6) 8 pt-g13) nil))
  (pt-vline (%+ x 12) (%+ y 1) 12 pt-white)
  (pt-hline (%+ x 1) (%+ y 12) 12 pt-white)
  nil)

;; ---------------------------------------------------------------- grow box
(define (pt-grow-box x y)
  (fill-rect x y 15 15 pt-g3)
  (pt-raised x y 15 15)
  (let ((i 0))
    (while (%< i 3)
      (let ((o (%+ 3 (%* i 4))) (j 0))
        (while (%< j 9)
          (let ((px (%+ x (%+ o j))) (py (%+ y (%- 11 j))))
            (if (%< (%- px x) 14)
                (begin (plot px py pt-g7) (plot px (%+ py 1) pt-white))
                nil))
          (set! j (%+ j 1))))
      (set! i (%+ i 1))))
  nil)
